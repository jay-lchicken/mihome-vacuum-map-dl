		...
		return new Promise((resolve, reject) => {
			...
			request(options, (error, response, body) => {
				if (!error && response && response.statusCode === 200) {
					...
					const handleCaptchaAndLogin = (captchaCode) => {
						...
						request(options, (error, response, body) => {
							if (!error && response && response.statusCode === 200) {
								...
								if (jsonData.code) {
									...
									if (jsonData.captchaUrl && this.adapter.getCaptchaCode) {
										this.adapter.log.info('Captcha required for login');
										const captchaUrl = 'https://account.xiaomi.com' + jsonData.captchaUrl;
										this.adapter.getCaptchaCode(captchaUrl)
											.then(code => {
												this.adapter.log.debug('Retrying login with captcha code');
												handleCaptchaAndLogin(code);
											})
											.catch(err => {
												this.adapter.log.error('Failed to get captcha code: ' + err);
												reject('Login failed - captcha required but not provided');
											});
									} else {
										this.adapter.log.error('Login failed with code: ' + jsonData.code + ' - ' + jsonData.description);
										reject('Login failed');
									}
									return;
								}
