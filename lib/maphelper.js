...
class MapHelper {
	...
	login(username, password) {
		...
		return new Promise((resolve, reject) => {
			...
			this.adapter.log.debug('Xiaomi Cloud: Starting OAuth login');

			const callbackPort = 8765;
			const callbackUrl = `http://localhost:${callbackPort}/callback`;
			
			// Build the Xiaomi OAuth URL
			const sid = 'xiaomiio';
			const oauthUrl = `https://account.xiaomi.com/pass/serviceLogin?sid=${sid}&callback=${encodeURIComponent(callbackUrl)}&_json=true`;
			...
			const handleCallback = (req, res) => {
				...
				if (parsedUrl.pathname === '/callback') {
					this.adapter.log.debug('Callback received: ' + req.url);
					this.adapter.log.debug('Query params: ' + JSON.stringify(parsedUrl.query));
					...
					const { ssecurity, userId: uid, cUserId, serviceToken: svcToken, code, nonce, location } = parsedUrl.query;
					
					if (svcToken) {
						// Direct service token in callback
						serviceToken = svcToken;
						if (ssecurity) sSecurity = ssecurity;
						if (uid) userId = uid;
						this.adapter.log.debug('Xiaomi Cloud: Login successful');
						this.ready = true;
						resolve(true);
					} else if (location) {
						// Need to follow a location URL
						this.adapter.log.debug('Following location from callback: ' + location);
						...
						request(options, (error, response, body) => {
							if (!error && response && response.statusCode === 200) {
								const cookies = j.getCookies(location.startsWith('http://') ? location : 'https://sts.api.io.mi.com/');
								
								this.adapter.log.debug('Cookies from location: ' + JSON.stringify(cookies));
								...
								if (found) {
									...
									reject('Service token not found in location response');
								}
							} else {
								reject('Error following location URL');
							}
						});
					} else {
						// Try to extract from URL hash or other parameters
						reject('No service token or location found in callback');
					}
				} else {
			...
			server.listen(callbackPort, (err) => {
				if (err) {
					return reject('Failed to start callback server: ' + err);
				}

				this.adapter.log.info('------------------');
				this.adapter.log.info('Xiaomi Cloud OAuth Login');
				this.adapter.log.info('------------------');
				this.adapter.log.info('Please open the following URL in your browser to authenticate:');
				this.adapter.log.info('');
				this.adapter.log.info(oauthUrl);
				this.adapter.log.info('');
				this.adapter.log.info('Waiting for authentication callback...');
				this.adapter.log.info('------------------');
			});
			...
			setTimeout(() => {
				if (server && server.listening) {
					server.close();
					reject('Login timeout - no callback received within 5 minutes');
				}
